// $Id: nmonitor.c,v 1.2 2018/07/27 16:59:51 karn Exp karn $
// Listen to multicast group(s), send audio to local sound device via portaudio
// Experimental: this version uses blocking portaudio rather than callbacks
...
void *decode_task(void *arg){
  ...
  while(!sp->terminate){
    ...
    float (*bounce)[2];
    bounce = (float (*)[2])malloc(sp->frame_size * 2 * sizeof(float));
    switch(sp->type){
    case PCM_STEREO_PT:
      for(int i=0; i < sp->frame_size; i++){
	// Redo delays
	bounce[i][0] = SCALE * (signed short)ntohs(*data_ints++) * left_gain;
	bounce[i][1] = SCALE * (signed short)ntohs(*data_ints++) * right_gain;
      }
      break;
    case PCM_MONO_PT:
      for(int i=0; i < sp->frame_size; i++){
	float s = SCALE * (signed short)ntohs(*data_ints++);
	bounce[i][0] = s * left_gain;
	bounce[i][1] = s * right_gain;
      }
      break;
    case OPUS_PT:
    case 20:
